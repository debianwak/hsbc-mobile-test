...
import com.globant.training.mobile.util.TemeratureUtil;
...
public class WeatherModelConverterImpl implements WeatherModelConverter {
    ...
    @Inject
    private TemeratureUtil temeratureUtil;
    ...
    public WeatherResponse weatherApiModelToResponseModel(WeatherAPIModel apiModel) {
        ...
        return GenericBuilder.of(WeatherResponse::new)
                ...
                .with(WeatherResponse::setTodayDate, dateTimeUtil.convertDateToString(MM_DD_YYYY, new Date()))
                .with(WeatherResponse::setDescWeather, Optional.ofNullable(apiModel.getWeather()).map(w -> w.get(0).getDescription()).orElseGet(() -> ""))
                .with(WeatherResponse::setTempCelsius, Optional.ofNullable(apiModel.getMain()).map( m-> temeratureUtil.convertKevinToCelsius(m.getTemp())).orElseGet(() -> 0D))
                .with(WeatherResponse::setTempFahrenheit, apiModel.getMain() != null ? temeratureUtil.convertKevinToFahrenheit(apiModel.getMain().getTemp()): 0D)
                .with(WeatherResponse::setSunrise, apiModel.getSys() != null?
                            dateTimeUtil.convertUnixDateToString(HH_MM_A, apiModel.getSys().getSunrise()): null)
                .with(WeatherResponse::setSunset, apiModel.getSys() != null?
                            dateTimeUtil.convertUnixDateToString(HH_MM_A, apiModel.getSys().getSunset()): null)
                .build();
    }
