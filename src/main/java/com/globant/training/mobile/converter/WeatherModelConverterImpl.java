...
public class WeatherModelConverterImpl implements WeatherModelConverter {
    ...
    public WeatherResponse weatherApiModelToResponseModel(WeatherAPIModel apiModel) {
        ...
        return GenericBuilder.of(WeatherResponse::new)
                ...
                .with(WeatherResponse::setTempCelsius, Optional.ofNullable(apiModel.getMain()).map( m-> temperatureUtil.convertKevinToCelsius(m.getTemp())).orElseGet(() -> 0D))
                .with(WeatherResponse::setTempFahrenheit, apiModel.getMain() != null ? temperatureUtil.convertKevinToFahrenheit(apiModel.getMain().getTemp()): 0D)
